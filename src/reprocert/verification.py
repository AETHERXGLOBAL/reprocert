...
def verify_certificate(
    ...
    *,
    ...
    checks.append(
        _result(
            "verdict_consistency",
            certificate.get("verdict") == logical,
    ...
    return {
        ...
        "checks": checks,
        "trust_boundary": (
            "This verification checks certificate structure, internal "
            "consistency, and optional local file digests. It does not "
            "authenticate the producer. Verify a separate signed CI "
            "attestation when producer identity matters."
        ),
    }


def _expected_verdict(certificate: dict[str, Any]) -> str:
    run = certificate.get("run", {})
    if run.get("execution_error") or run.get("timed_out"):
        return "ERROR"

    accepted = run.get("accepted_exit_codes")
    if not isinstance(accepted, list) or not accepted:
        accepted = [run.get("expected_exit_code", 0)]
    if run.get("exit_code") not in accepted:
        return "ERROR"

    checks = certificate.get("checks", [])
    if certificate.get("diagnostics") or any(
